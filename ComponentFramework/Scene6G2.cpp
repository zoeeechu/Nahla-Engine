...
// this code is diabolical it makes me wanna shoot myself
// but it works                                                 -zoe 
// gosh i hate physics code 
...
static string currentScene = "Scene6G2";
...
static Entity &sphere(manager.addEntity("sphere"));
...
std::vector<Entity> tentacleSpheres;

Scene6G2::Scene6G2() : drawInWireMode(false), Skulltexture(nullptr), Eyetexture(nullptr)
{
	Debug::Info("Created Scene6G2: ", __FILE__, __LINE__);
}

Scene6G2::~Scene6G2()
{
	Debug::Info("Deleted Scene6G2: ", __FILE__, __LINE__);
}

bool Scene6G2::OnCreate()
{
	...
	Debug::Info("Loading assets Scene6G2: ", __FILE__, __LINE__);
	...
	skull.addComponent<newTransformComponent>(Vec3(0.0f, 0.5f, 3.0f), Vec3(0.15f, 0.15f, 0.15f), Quaternion(1.0f, Vec3(0.0f, 1.0f, 0.0f)));
	skull.addComponent<TextureComponent>().LoadTexture("textures/skull_texture.jpg");

	sphere.addComponent<newMeshComponent>(nullptr, "meshes/sphere.obj");
	...
	{
		anchors.push_back(Entity(manager.addEntity("anchor" + std::to_string(i))));
		anchors[i].addComponent<BodyComponent>();
		anchors[i].getComponent<BodyComponent>().pos = anchorPos;
		anchors[i].getComponent<BodyComponent>().radius = 0.05f;

		for (int j = 0; j < numSpheresPerAnchor; j++)
		{
			tentacleSpheres.push_back(Entity(manager.addEntity("Tentaclesphere" + std::to_string(i * 10 + j))));
			tentacleSpheres[j + i * 10].addComponent<BodyComponent>();
			tentacleSpheres[j + i * 10].getComponent<BodyComponent>().pos = anchorPos + Vec3(0, -(j + 1) * spacing, 0);
			tentacleSpheres[j + i * 10].getComponent<BodyComponent>().radius = 0.02f;
		}
...
void Scene6G2::OnDestroy()
{
	Debug::Info("Deleting assets Scene6G2: ", __FILE__, __LINE__);
...
void Scene6G2::HandleEvents(const SDL_Event &sdlEvent)
{
	...
	case SDL_KEYDOWN:
		...
		case SDL_SCANCODE_UP:
			...
			{
				anchor.getComponent<BodyComponent>().vel.y = speed;
			}
			break;
		...
		case SDL_SCANCODE_RIGHT:
			for (Entity &anchor : anchors)
			{
				anchor.getComponent<BodyComponent>().vel.x = speed;
			}
			break;
	...
	case SDL_KEYUP:
		for (Entity &anchor : anchors)
		{
			anchor.getComponent<BodyComponent>().vel = Vec3(0, 0, 0);
		}
		break;
...
void Scene6G2::Update(const float deltaTime)
{
	if (IsPaused)
	{
		return;
	...
	for (int i = 0; i < 100; i++)
	{
		Vec3 gravityForce = Vec3(0, -9.8f * tentacleSpheres[i].getComponent<BodyComponent>().mass, 0);
		Vec3 dragForce = -5.0f * tentacleSpheres[i].getComponent<BodyComponent>().vel;
		Vec3 netForce = gravityForce + dragForce;
		tentacleSpheres[i].getComponent<BodyComponent>().ApplyForce(netForce);
		// Calculate initial estimate of velocity from gravity & drag
		tentacleSpheres[i].getComponent<BodyComponent>().UpdateVel(deltaTime);
		// Constraint goes here:

		if (i % 10 == 0)
		{
			tentacleSpheres[i].getComponent<BodyComponent>().RodConstraint(deltaTime, anchors[i / 10].getComponent<BodyComponent>().pos, tentacleSpheres[i].getComponent<BodyComponent>().rodLength);
		}
		else
		{
			tentacleSpheres[i].getComponent<BodyComponent>().RodConstraint(deltaTime, tentacleSpheres[i - 1].getComponent<BodyComponent>().pos, tentacleSpheres[i].getComponent<BodyComponent>().rodLength);
		}

		tentacleSpheres[i].getComponent<BodyComponent>().UpdatePos(deltaTime);
	}
	for (Entity &anchor : anchors)
	{

		anchor.getComponent<BodyComponent>().UpdatePos(deltaTime);
	}
}

void Scene6G2::HandleTheGUI()
{

	bool open = true;

	ImGui::Begin("map", &open, ImGuiWindowFlags_NoResize);
	ImGui::Text(" Hey scott! use arrow keys to move jelly fish :D");
	ImGui::End();
...
void Scene6G2::Render() const
{
	...
	for (Entity &anchor : anchors)
	{
		glUniformMatrix4fv(shader.getComponent<newShaderComponent>().GetUniformID("modelMatrix"), 1, GL_FALSE, MMath::toMatrix4(trackball.getQuat()) * anchor.getComponent<BodyComponent>().GetModelMatrix());
		// sphere.getComponent<newMeshComponent>().Render(GL_TRIANGLES);
		anchor.active ? sphere.getComponent<newMeshComponent>().Render(GL_TRIANGLES) : void();
	}

	for (Entity &tentacleSphere : tentacleSpheres)
	{

		glUniformMatrix4fv(shader.getComponent<newShaderComponent>().GetUniformID("modelMatrix"), 1, GL_FALSE, tentacleSphere.getComponent<BodyComponent>().GetModelMatrix());
		// sphere.getComponent<newMeshComponent>().Render(GL_TRIANGLES);
		tentacleSphere.active ? sphere.getComponent<newMeshComponent>().Render(GL_TRIANGLES) : void();
	}
